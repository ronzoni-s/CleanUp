            ...
            _poliboxCards = order.OrderPoliboxs.Select(x => new Polibox()
            {
                ...
                Bags = x.Bags
                    .Select((bag, index) => bag.Status switch {
                        CleanUp.Application.Models.BagStatus.Full => new BagState(false, true, index + 1),
                        //CleanUp.Application.Models.OrderPolibox.BagStatus.Order => order.CompletionDateTime.HasValue || order.CancellationDateTime.HasValue
                        //    ? new BagState(false, true)
                        //    : new BagState(true, false),
                        CleanUp.Application.Models.BagStatus.Order => new BagState(true, false, index + 1),
                        CleanUp.Application.Models.BagStatus.Empty => new BagState(false, false, index + 1),
