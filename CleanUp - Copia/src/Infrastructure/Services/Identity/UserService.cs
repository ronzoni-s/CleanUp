...
using AutoMapper;
using ErbertPranzi.Application.Exceptions;
...
using ErbertPranzi.Infrastructure.Specifications;
using ErbertPranzi.Shared.Constants.Role;
...
namespace ErbertPranzi.Infrastructure.Services.Identity
{
        ...
        public async Task<string> ExportToExcelAsync(string searchString = "")
        {
            ...
            var result = await _excelService.ExportAsync(users, sheetName: _localizer["Users"],
                mappers: new Dictionary<string, Func<BlazorHeroUser, object>>
                {
                    ...
                    { _localizer["CreatedOn (Local)"], item => DateTime.SpecifyKind(item.Created, DateTimeKind.Utc).ToLocalTime().ToString("G", CultureInfo.CurrentCulture) },
                    { _localizer["CreatedOn (UTC)"], item => item.Created.ToString("G", CultureInfo.CurrentCulture) },
